...
1. **Discover before you choose (free).** Discover each dimension you are about to
   ...
   When multiple refs in a category could fit, run
   `pnpm cli refs:contact-sheet --category <category> --out <dir>` or inspect
   the relevant `_thumb.png` previews before picking. Do not choose a
...
2. **Pick defaults.** Choose a preset that fits the topic (see
   ...
   `pnpm cli list-models`. State the choices to the user; don't over-ask.
