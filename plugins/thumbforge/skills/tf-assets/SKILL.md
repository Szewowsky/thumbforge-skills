...
pnpm cli refs:rethumb --category character-primary  # rebuild display thumbs
pnpm cli refs:contact-sheet --category character-primary --out /tmp/thumbforge-ref-sheets
```
...
`<id>_thumb.png`. For a category with many candidates, run
`refs:contact-sheet` and inspect the written PNG; for a few candidates, reading
thumbs directly is fine. If face thumbnails look stale or center-cropped, run
`pnpm cli refs:rethumb --category character-primary` (or
