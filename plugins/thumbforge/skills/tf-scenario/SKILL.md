...
If refs are not obvious from the list, use `thumbforge list-refs` as the
thin-client index and inspect the relevant `_thumb.png` previews before putting
them in a concept. In repo/dev only, you may also run
`pnpm cli refs:contact-sheet --category <category> --out <dir>`. Pick by visible
